...
public class OnlineCheck implements CommandExecutor {
	...
	private void online_check(CommandSender sender) {
		...
		AntiAfk.instance.getLogger().info(msg.toPlainText());
		for (Player p : AntiAfk.instance.getServer().getOnlinePlayers()) {
	...
	private void kick_afk() {
		AntiAfk.instance.getLogger().info("Kicking AFK people");

		for (UUID uuid : waiting_for) {
			Player player = AntiAfk.instance.getServer().getPlayer(uuid);
			if (player == null) {
				continue;
			}
